...
### 3.1 临时存储策略
本项目采用 **“临时持久化”** 策略。SQLite 仅作为运行时的数据索引和缓冲工具，以支持 QML 高性能列表渲染。所有本地生成的数据库文件在程序正常退出时会被物理删除，确保“阅后即焚”。
...
从 v1.6.0 开始，系统全面采用**数据库单一数据源（Single Source of Truth）**架构，彻底消除"内存+数据库"双重维护的复杂性和数据不一致风险。

### 7.2 核心组件重构

#### 7.2.1 UserManager (用户管理器)

**重构前**：
- 维护内存字典 `self.users: Dict[str, User]`
- 同时需要与数据库同步数据
- 存在数据不一致隐患
...
class UserManager:
    ...
    def get_user(self, user_id: str) -> Optional[User]:
        ...
        result = self.db_manager.query_one(sql, (user_id,))
...
#### 7.2.2 MessageListModel (消息列表模型)

**重构前**：
- 维护内存列表 `self._messages: List[dict]`
- 提供 `add_message()` 和 `set_messages()` 方法
- 需要在发送/接收时手动更新列表
...
class MessageListModel(QAbstractListModel):
    def __init__(self, db_manager=None, parent=None):
        ...
        # 移除了 self._messages 列表
    
    def rowCount(self, parent=None):
        """实时从数据库查询消息数量"""
        if self._chat_type == 'user':
...
**关键机制**：
- QML 每次请求数据时，`rowCount()` 和 `data()` 自动调用
- 调用 `refresh()` 触发 `beginResetModel()`/`endResetModel()`
- QML 重新调用 `rowCount()` 和 `data()` 获取最新数据

### 7.3 消息接收流程修复

**问题**：重构后对方发送的消息不显示，因为未保存到数据库。
...
**完整流程**：
```
1. MessageService 接收 TCP 消息
   ↓
2. _on_message_received_raw()
   ├─ db.save_message()         ← 保存到数据库
   ├─ userListChanged.emit()    ← 更新未读计数
   └─ _internalMessageSignal.emit()
   ↓
3. process_received_message()
   ├─ 判断是否为当前会话
   ├─ mark_as_read()            ← 如果是当前会话，标记已读
   └─ msg_model.refresh()       ← 刷新 UI
   ↓
4. MessageListModel.refresh()
   ├─ beginResetModel()
   └─ endResetModel()
   ↓
5. QML 自动调用 rowCount() 和 data()
   └─ 从数据库实时查询最新消息
```
...
**查询频率**：
- QML ListView 采用虚拟化渲染，只查询可见区域的数据
- SQLite 查询性能对于局域网聊天场景足够（通常 < 1000 条消息）
- 已创建索引优化查询性能
...
**文档版本**: v1.6.0  
**创建日期**: 2026-01-21  
**最后更新**: 2026-01-21 (新增数据库单一数据源架构章节)
