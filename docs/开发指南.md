...
### ✅ UI 界面
- [x] 主窗口（MainWindow）
- [x] 三栏式现代布局 (Sidebar / Contact / Chat)
- [x] 无边框 (Frameless) 窗口与自定义拖拽
- [x] FontAwesome 矢量图标集成 (CDN 加载)
- [x] 消息气泡美化与头像显示
- [x] 创建群组对话框与成员选中逻辑 (selectedIds Map)
...
### 群组聊天
- [x] 创建群组
- [x] 群组成员管理 (GROUP_INVITE 广播邀请)
- [x] 群组消息广播 (UDP 组播 + 自环过滤)
...
### 测试多用户
应用支持 **SO_REUSEPORT**，你可以在同一台电脑或不同终端窗口运行多个实例进行互通测试：

```bash
# 开启第一个实例
python run.py

# 开启第二个实例 (通过环境变量隔离端口与数据)
TCP_PORT=10001 DATA_DIR=./data2 python run.py
```
...
├── ui/                  # 界面模块
│   ├── qml/             # QML 视图文件
│   ├── controllers/     # 业务子控制器 (解耦逻辑)
│   ├── models/          # QML 数据模型
│   └── backend.py       # 后端桥接/调度入口
├── database/            # 数据库
│   └── db_manager.py    # 数据库管理
└── utils/               # 工具类
    └── logger.py        # 日志工具
